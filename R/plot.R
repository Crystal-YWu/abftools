...
#' @param conductance whether to plot conductance instead of current. Default is
#' to plot conductance if Conductance is found in
#' @param colour whether to plot in colour mode.
#' @param title title of the plot.
#' @param legend_title title of the legend (group).
#' @param error_bar whether to plot error bars. Default is TRUE unless plotting
#' conductance.
#' @param zero_axes wheter to apply ZeroAxes() to plot.
#' @param smooth wheter to plot b-Spline smoothed lines.
#' @param ... not used.
#'
#' @return a ggplot object.
#' @export
#'
PlotIVSummary <- function(ivs, conductance = "Conductance" %in% names(ivs),
  ...
  if (error_bar) {
    ...
    p <- p +
      geom_point() +
      geom_errorbar(aes_string(ymin = sprintf("%s - %s", ycol, ysem),
                               ymax = sprintf("%s + %s", ycol, ysem)),
                    width = delta / 8)
  }
  ...
  if (smooth) {
    if (colour) {
      p <- p + geom_smooth(se = FALSE, method = "lm",
                           formula = y ~ splines::bs(x, min_pts - 1L))
    } else {
